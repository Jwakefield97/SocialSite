...
$(document).ready(function(){$("#test").text($(window).width()+"--"+window.visualViewport.width+"--"+window.innerWidth);var a=document.getElementsByTagName("html")[0],c,b="pills-feed-tab";a.addEventListener("touchstart",function(a){var b=a.target.nodeName.toLowerCase();"tbody"!==b&&"thead"!==b&&"table"!==b&&"td"!==b&&"tr"!==b&&"img"!==b&&"a"!==b&&(c=a.touches[0].clientX)},!1);a.addEventListener("touchend",function(a){var d=a.changedTouches[0].clientX-c;a=a.target.nodeName.toLowerCase();"tbody"!==
a&&"thead"!==a&&"table"!==a&&"td"!==a&&"tr"!==a&&"img"!==a&&"a"!==a&&(0>d?65<Math.abs(d)&&("pills-feed-tab"===b?(b="pills-search-tab",$("#pills-search-tab").click()):"pills-search-tab"===b?(b="pills-friends-tab",$("#pills-friends-tab").click()):"pills-friends-tab"===b?(b="pills-profile-tab",$("#pills-profile-tab").click()):"pills-profile-tab"===b&&(b="pills-profile-tab",$("#pills-profile-tab").click())):65<Math.abs(d)&&("pills-profile-tab"===b?(b="pills-friends-tab",$("#pills-friends-tab").click()):
"pills-friends-tab"===b?(b="pills-search-tab",$("#pills-search-tab").click()):"pills-search-tab"===b?(b="pills-feed-tab",$("#pills-feed-tab").click()):"pills-feed-tab"===b&&(b="pills-feed-tab",$("#pills-feed-tab").click())));c=0},!1);if(550>$(window).width()||550>window.visualViewport.width||550>window.innerWidth)$("#usersFullTable").addClass("table-responsive"),$("#friendsPendingFullTable").addClass("table-responsive"),$("#friendsFullTable").addClass("table-responsive");$(window).resize(function(a){550>
$(window).width()||550>window.visualViewport.width||550>window.innerWidth?($("#usersFullTable").addClass("table-responsive"),$("#friendsPendingFullTable").addClass("table-responsive"),$("#friendsFullTable").addClass("table-responsive")):($("#usersFullTable").removeClass("table-responsive"),$("#friendsPendingFullTable").removeClass("table-responsive"),$("#friendsFullTable").removeClass("table-responsive"))});getPosts();$('[data-toggle="tooltip"]').tooltip();$("#createPost").click(function(a){$("#modalPostText").val("");
$("#createPostModal").modal("show")});$("#pills-feed-tab").click(function(a){b="pills-feed-tab";getPosts()});$("#createPostModalButton").click(function(a){$("#createPostModal").modal("hide");a=$("#modalPostText").val().trim();0===a.length&&2E3===a.length||$.ajax({type:"POST",url:"/home/createPost",data:{post_text:a},success:function(a){"error"!==a?(getPosts(),setMessage("success","post was created successfully!")):setMessage("error","An error occurred while creating post.")}})});$("#pills-friends-tab").click(function(a){b=
"pills-friends-tab";getPendingRequests();getFriends()});$("#pills-search-tab").click(function(a){b="pills-search-tab";userCount=0;userAmount=15;getUsers()});$("#loadMoreUsers").click(function(a){getUsers()});$("#searchSiteButton").click(function(a){searchUserTable()});$("#searchSiteBar").keypress(function(a){"Enter"===a.key&&searchUserTable()});$("#sendFriendRequest").click(function(a){sendFriendRequest($("#inspectUsername").text())});$("#pills-profile-tab").click(function(a){b="pills-profile-tab";
getAdditonalInfo()});$("#imageLink").change(function(a){$("#profileImage").attr("src",$("#imageLink").val().trim())});$("#saveChanges").click(function(a){a={bio:$("#bio").val().trim(),email:$("#email").val().trim(),phoneNumber:$("#phoneNumber").val().trim(),profileImage:$("#imageLink").val().trim()};$.ajax({type:"POST",method:"POST",url:"/home/updateProfile",data:a,success:function(a){"success"===a?setMessage("success","Your profile was update successfully!"):setMessage("error","There was an error updating your profile.")}})})});